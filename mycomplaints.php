...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $complaintType = $complaintDescription = $email = $complaintStatus = $complaintOccuredDate = $complaintResolvedDate = "";

    // Validate and sanitize input
    $complaintType = trim($_POST["ticketType"]);
    $complaintDescription = trim($_POST["ticketDescription"]);
    $email = trim($_POST["ticketEmail"]);

    // Check if "Other" option is selected and get new complaint type
    if ($complaintType == "Other") {
        $newComplaintType = trim($_POST["newComplaintType"]);
        // Use the new complaint type if provided, otherwise, use "Other"
        $complaintType = !empty($newComplaintType) ? $newComplaintType : "Other";
    }

    // Set default values
    $complaintStatus = "P"; // Default status is "Pending"
    $complaintOccuredDate = date("Y-m-d H:i:s"); // Get current date and time

    // Prepare and execute SQL statement to insert data into the database
    $sql = "INSERT INTO complaints (complaint_type, complaint_description, email, complaint_status, complaint_occured_date, complaint_resolved_date) VALUES (?, ?, ?, ?, ?, ?)";
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("ssssss", $complaintType, $complaintDescription, $email, $complaintStatus, $complaintOccuredDate, $complaintResolvedDate);
        if ($stmt->execute()) {
...
<body>
    ...
    <aside id="sidebar" class="sidebar">
        <ul class="sidebar-nav" id="sidebar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link " href="./mycomplaints.php">
